...
def fetch_training_data() -> pd.DataFrame:
    ...
    with database_engine().connect() as conn:
        dataset: pd.DataFrame = pd.read_sql(
...
def fetch_evaluation_data() -> pd.DataFrame:
    ...
    with database_engine().connect() as conn:
        try:
            ...
            evaluation_dataset: pd.DataFrame = pd.read_sql(
...
def fetch_trained_model_data() -> list:
    ...
    with database_engine().connect() as conn:
        ...
        model_data_result = conn.execute(
            text(
                """
                    SELECT 
                        model_info.id, 
...
def upload_to_gcs(bucket_name, source_file_path, destination) -> None:
...
def remove_models(path: str, query: str) -> None:
...
def update_model_info(model_info_list: list, data_transformer_list: list) -> None:

    with database_engine().connect() as conn:
        
        try:
            for data_transformer in data_transformer_list:
                data_transformer_id_result = conn.execute(
                    text(
                        """
                        SELECT 
                            id 
        ...
        try:
            for model_info in model_info_list:
                ...
                model_info_id_result = conn.execute(
                    text(
                        """
